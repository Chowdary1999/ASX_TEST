...
// Faster, timeout-aware pick3 with fallback ranking to avoid hangs.
...
async function pick3Fast(mode='fast'){
  ...
  const pool = rows.sort((a,b)=>(b.mcap||0)-(a.mcap||0)).slice(0, limit);
  ...
  async function worker(){
    while(idx < pool.length && Date.now() < deadline && out.length < 12){
      const row = pool[idx++];
      try{
        const r = await scoreWithYahoo(row.symbol, row.mcap, 2000);
  ...
  if (out.length < 3){
    ...
    return { picks, asOf:new Date().toISOString(), note:'fallback: TE deltas' };
  }

  out.sort((a,b)=> (b.score||-1e9)-(a.score||-1e9));
  const picks = out.slice(0,3).map((t,i)=>({ ...t, rank:i+1 }));
  return { picks, asOf:new Date().toISOString(), note:'yahoo momentum' };
}
...
exports.handler = async (event) => {
  try {
    ...
    const result = await pick3Fast(mode);
    return json(result);
  } catch (e) {
